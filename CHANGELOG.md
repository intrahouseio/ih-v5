...
# development
- Добавлена функционал промежуточных beta версий. 

# 5.5.36 15-07-2021
- В редакторе визуальных элементов при показе дерева устройств, если нет привязки, позиционирование выполняется на последнее устройство, для которого была выполнена привязка.
- Для операции gotoLayout изменены имена свойств (container_id, device_id). Привязки в старом формате работать не будут
- Исправлен запрос trend для устройства-контекста

# 5.5.35 14-07-2021
- Добавлена функция "Перепривязать" в редакторе визуальных элементов. Выполняет привязку к устройству по аналогии с уже сделанными привязками. 
  Механизм использования:
  - выполнить привязки свойств устройства (и команд) к элементу визуализации (шаблону, примитиву,..)   
  - копировать элемент
  - в popup меню по правой кнопке мыши выбрать операцию "Перепривязать", в диалоге выбрать устройство
  Будут выполнены привязки к свойствам выбранного устройства. Операция также может быть выполнена для группы элементов
  
- Реализован механизм отправки значений свойств устройств на плагины по подписке (публикация данных системы, например, по mqtt) 
- Выпущена версия плагина mqtt 5.5.5
  - Исправлена ошибка при отправке команд управления
  - Восстановлена вкладка "Расширения" для формирования списка устройств для публикации с указанием qos, retain 
  - Публикация данных из списка "Расширения" при получении данных от сервера по подписке
  - Изменения при формировании каналов, добавленных процедурой сканирования  

# 5.5.34 12-07-2021
- Реализован механизм переключения между экранами с изменением содержимого контейнеров и устройств-контекстов. 
- Технические изменения в PM 

# 5.5.33 09-07-2021
- Добавлен протокол обмена и диалог для сканирования каналов (для плагинов, имеющих такую функцию).
  На текущий момент сканирование реализовано в плагине mqtt
- Добавлен раздел Скрипты визуализации и механизм запуска скриптов. 
  Эти скрипты используют контекст текущего экрана (например, для переключения экранов/фреймов в зависимости от локальных переменных или элементов экрана)
- Добавлена возможности по аналогии с диалогом в привязках контейнера использовать опцию "Любое устройство". 
  Привязка к фактическому устройству выполняется при установке на экран, или при выполнении команды переключения экрана/фрейма, или с помощью скрипта визуализации  

# 5.5.31 23-06-2021
- Добавлен новый виджет Chart Bar 
- Изменения в механизме отладчика плагина
- Добавлена выгрузка лога плагина
- Добавлен import/export devicedb через csv
- Добавлена обработка манифеста для dbagent (ext свойства)
- Исправлена ошибка: при копировании устройства, содержащего правила записи в БД, запись в БД начиналась только после редактирования правила или после перезагрузки
- Исправлена ошибка: при изменении типа устройства оставались свойства предыдущего типа в правилах записи в БД и в списке свойств для визуализации
- Исправлена ошибка: некорректный набор изображений при выборе изображения в диалоге при работе через p2p

# 5.5.30 23-06-2021
- Добавлен фильтр в отладчике

# 5.5.28 22-06-2021
- Доработана форма Плагины на дашборде
- Отдельный лог для каждого плагина. Настройка уровня сообщений для логирования

# 5.5.27 21-06-2021
- Добавлена вкладка Плагины на дашборд для обновления и установки плагинов
- Изменены структуры для manifest.json: плагин определяет popup для каналов 

# 5.5.26 18-06-2021
- Добавлена возможность копирования экземпляра плагина включая каналы
- Изменен манифест плагинов: manifest.json
- Добавлена возможность в манифесте задавать структуру каналов (папка, узел, канал, канал для чтения, канал для записи)  
- Новая версия системного плагина sqlite (модуль calculate - расчет времени работы)

# 5.5.24 15-06-2021
- Изменения связанные с масштабированием графиков 

# 5.5.23 11-06-2021
- Выпуск десктопной версии клиента 

# 5.5.22 09-06-2021
- Новая версия системного плагина p2p 

# 5.5.21 09-06-2021
- Добавлен multichart для графиков в IntraHouse при наличии лицензии

# 5.5.20 09-06-2021
- Обновление версии в системных индикаторах (процессах) после обновления плагина
- Остановленный плагин теперь не стартует при перезапуске сервера 
- Исправлена ошибка, связанная с calc обработчиками
- Исправлена закачка плагинов для Windows
